...
;(function($,window,document,undefined)
{
	...
	animateSlider.prototype = 
	{
		...
		autoplay	:	function() 
		{
			...
			{
				var that				=	this;
				var loop = function(){that.autoplayNew()};
				this.autoplayTime = setInterval(loop,this.config.interval);
			...
			// 		dkm =	setTimeout(loop,this.interval[page]);
			// 		return;
			// 	}
			...
			// 			// this.autoplayTime	=	setTimeout(loop,0);
			// 			dkm = setTimeout(loop,0);		
			// 		}				
			// 		else if( animationTime < that.config.interval	)
			// 		{
			// 			that.interval[page]	=	that.config.interval;
			// 			// this.autoplayTime	=	setTimeout(loop,that.config.interval-animationTime);
			// 			dkm =	setTimeout(loop,that.config.interval-animationTime);
			// 		}
			// 	});
			// 	alert(dkm);
			}
		},
		autoplayNew : function()
		{
			if (this.config.autoplay)
			{
				var page = this.current;
				page	=	( page >= this.slidesCount -1 || page < 0 )	? 0 : page + 1;
				this.navigate(page);
			}
		},
		/**
		...
		animDuration:	function(page) 
		{
			...
			var slideAnimationsCount		=	$slideAnimations.find(".animated").length;
